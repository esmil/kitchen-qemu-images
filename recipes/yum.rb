...
cmd = [ 'yum', '-y', "--installroot=#{root}", '--disablerepo=*' ]
cmd.push("--releasever=#{node['image']['yum']['releasever']}") if node['image']['yum']['releasever']
node['image']['yum']['repos'].each do |repo|
